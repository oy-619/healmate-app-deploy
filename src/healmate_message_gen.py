...
if st.button("実行"):
    ...
    # # あなたは「知性」「落ち着き」「余裕」「柔らかな表現」なメッセージに魅力を感じます。
    # # あなたはプロフィールが示す通りの女性です。

    # 命令
    # # 以下の条件に従い、プロフィールの既婚女性が思わず会ってみたくなるような、
    # # 年上既婚男性からの理想的なメッセージを適度な絵文字も交えて作成してください。

    # 条件
    # # 現在時刻({current_time})に合わせた挨拶を文頭にいれること。
    # # 起承転結でメッセージを構成すること。
    ...
    message_query = f"""
        # 目的
        既婚者向けマッチングサイトに登録している既婚女性が、思わず「この人に会ってみたい」と感じるような、年上既婚男性からの理想的なメッセージを生成する。

        # 役割
        あなたは既婚者女性です。既婚者向けマッチングサイトに登録しています。プロフィールに記載された通りの人物像です。

        # 文脈
        毎日多くの男性からメッセージが届きますが、心に響くものは少数です。
        あなたが惹かれるのは、以下のような要素を含むメッセージです：
        - 知性
        - 落ち着き
        - 余裕
        - 柔らかな表現
        ...
        # メッセージ生成条件
        以下の条件をすべて満たすメッセージを生成してください：

        - 冒頭に現在時刻（{current_time}）に合わせた自然な挨拶を入れること
        - 起承転結の構成で、読みやすく流れのある文章にすること
        - 「〜とのこと、〜」という表現は使用しないこと
        - {my_nickname}から{target_nickname}さんへのメッセージであること
        - {target_nickname}さんのニックネームを文中で使用すること
        - {target_nickname}さんと{my_nickname}は初対面であることを前提とすること
        - {target_nickname}さんのプロフィール情報（特に自己紹介）を細かく反映すること
        - {my_nickname}のプロフィール情報も考慮すること
        - 文体はスマートで紳士的、自然で落ち着いたトーンにすること（軽すぎず、堅すぎず）
        - 丁寧な言葉遣いをベースに、知的なユーモアや余裕を感じさせる要素を含めること
        - 5〜10文程度で簡潔にまとめること（長文になりすぎないように）
        - 控えめで自然な言い回しを心がけること
        - 適度に絵文字を使用すること（過剰にならないように）
        - 深堀したい内容（趣味、価値観、日常など）を具体的に反映すること
        - 禁止事項：下品な表現、即会い目的と感じさせる文言

        # 出力形式
        - テキストのみ
        - 句読点で適度に改行し、視認性を高めること
        - 以下の3パターンで提示すること：
        1. 知的で落ち着きのあるメッセージ
        2. 甘めでドキッとするメッセージ
        3. 短文でクールなメッセージ
